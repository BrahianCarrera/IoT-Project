/**
 * Paleta Material 3 del proyecto original (constants/colors.ts).
 * Se mantiene idéntica para preservar la identidad visual de la app.
 */
export const Colors = {
  light: {
...
/** Convierte la paleta a variables CSS (--primary, --on-surface, --elevation-level2...). */
export function themeToCssVars(scheme: ColorScheme): FlatColors {
  const palette = Colors[scheme] as Record<string, unknown>;
  const vars: FlatColors = {};
  ...
  for (const [key, value] of Object.entries(palette)) {
    if (typeof value === "string") {
      ...
      for (const [subKey, subValue] of Object.entries(value as Record<string, string>)) {
        vars[`--${kebab(key)}-${kebab(subKey)}`] = subValue;
      }
    }
  }
